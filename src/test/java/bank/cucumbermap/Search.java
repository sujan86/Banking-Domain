...
HTMLReportGenerator.StepDetails(input6op.get("STATUS").toString(),"user select \"([^\"]*)\" as country", input6op.get("MESSAGE").toString());		 
	...
	@Then("^Application shows search result for \"([^\"]*)\" branches$")
	public void application_shows_search_result_for_branches(String result) throws Throwable
	{
		 Object[] search=new Object[2];
		 search[0]="(//*[text()='MUMBAI'])[1]";
		 search[1]=result;
		 Hashtable<String,Object>searchop= SeleniumOperations.validation(search);
HTMLReportGenerator.StepDetails(searchop.get("STATUS").toString(),"Application shows search result for \"([^\"]*)\" branches",searchop.get("MESSAGE").toString());	   
	}
